...
def load_telemetry(filename):
    times = []
    drones_data = [{'x': [], 'y': [], 'z': [], 'bat': []} for _ in range(N_DRONES)]
    try:
        with open(filename, 'r') as f:
            ...
            for row in reader:
                ...
                for d in range(N_DRONES):
                    ...
                    try:
                        ...
                        # Support old telemetry without battery by checking if key exists or using 100 as default
                        if f'bat{d}' in row:
...
def main():
    ...
    def update(frame):
        ...
        bat_strs = []
        for d in range(num_active_drones):
            if frame < len(drones_data[d]['bat']):
                bat = drones_data[d]['bat'][frame]
                bat_strs.append(f"D{d}: {bat:.0f}%")
